...
export const API = {
  ...
  // ── Content Plan (per-client editorial calendar + monthly approval links) ──
  cpClients:        () => request('/api/content-plan/clients'),
  cpCreateClient:   (body) => request('/api/content-plan/clients', { method: 'POST', body }),
  cpUpdateClient:   (id, body) => request('/api/content-plan/clients/' + id, { method: 'PATCH', body }),
  cpDeleteClient:   (id) => request('/api/content-plan/clients/' + id, { method: 'DELETE' }),
  cpItems:          (clientId, year, month) => request('/api/content-plan/items?client_id=' + clientId + '&year=' + year + '&month=' + month),
  cpItem:           (id) => request('/api/content-plan/items/' + id),
  cpCreateItem:     (body) => request('/api/content-plan/items', { method: 'POST', body }),
  cpUpdateItem:     (id, body) => request('/api/content-plan/items/' + id, { method: 'PATCH', body }),
  cpDeleteItem:     (id) => request('/api/content-plan/items/' + id, { method: 'DELETE' }),
  cpDuplicateItem:  (id, plan_date) => request('/api/content-plan/items/' + id + '/duplicate', { method: 'POST', body: { plan_date } }),
  cpUploadMedia:    (id, file, kind) => { const fd = new FormData(); fd.append('file', file); fd.append('kind', kind); return request('/api/content-plan/items/' + id + '/media', { method: 'POST', body: fd }); },
  cpDeleteMedia:    (mid) => request('/api/content-plan/media/' + mid, { method: 'DELETE' }),
  cpMediaUrl:       (mid, download) => url('/api/content-plan/media/' + mid + '/raw') + '?token=' + (getToken() || '') + (download ? '&download=1' : ''),
  cpAddComment:     (id, body) => request('/api/content-plan/items/' + id + '/comments', { method: 'POST', body: { body } }),
  cpShares:         (clientId) => request('/api/content-plan/shares?client_id=' + clientId),
  cpCreateShare:    (body) => request('/api/content-plan/shares', { method: 'POST', body }),
  cpUpdateShare:    (id, body) => request('/api/content-plan/shares/' + id, { method: 'PATCH', body }),
  cpDeleteShare:    (id) => request('/api/content-plan/shares/' + id, { method: 'DELETE' }),
  // public (customer side)
  cpPublic:         (token, pw) => request('/api/cp/' + token + (pw ? '?p=' + encodeURIComponent(pw) : ''), { skipAuth: true }),
  cpPublicUnlock:   (token, password) => request('/api/cp/' + token + '/unlock', { method: 'POST', body: { password }, skipAuth: true }),
  cpPublicMediaUrl: (token, mid, pw, download) => url('/api/cp/' + token + '/media/' + mid) + '?' + (pw ? 'p=' + encodeURIComponent(pw) + '&' : '') + (download ? 'download=1' : ''),
  cpPublicReview:   (token, id, body, pw) => request('/api/cp/' + token + '/items/' + id + '/review' + (pw ? '?p=' + encodeURIComponent(pw) : ''), { method: 'POST', body, skipAuth: true }),
